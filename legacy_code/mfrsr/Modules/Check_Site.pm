...
#       associated directory for each active station.  No arguments 
...
#
#    If called with no arguments, returns array references to:
#      0. Valid_List with directories
#      1. Unique TLIs, active or inactive 
#
# Known Shortcomings:
#
...
# Must check different paths as module is called from different machines
    my $file;
    my @attpaths = ('/mnt', '/home/grad');
    foreach my $attpath (@attpaths) {
	$file = "$attpath/mfrsr/Calinfo/site_attributes";
	next if ! -e $file;
	open SI, '<', $file or die "\n Can't open $file: $!\n\n";
    }
...
# Sanity tests on inputs ensure validly formed tli or tli_type
    ...
    if (scalar@_ > 0) {
	foreach my $site (@_) {
#	    my $n = scalar(split /\_/,$tmp); # Test if TLI or TLI_TYPE
	    my @n = split /\_/,$site;
	    my $n = scalar@n;
	    if ($n == 2) {
	    ...
	    } elsif ($n == 1) {     # TLI only, no underscore
		if ($site !~ /^[a-z0-9]{3}$/) {
		    &usage0($site,$file);
		    die "\n";
		}
...
# @plist contains only TLI_TYPEs listed as active in attributes file.
#        along with the appropriate directrory.  For processing only.
# @tlist contains all TLI_TYPEs in attributes file without directory
#        information.  Used for MFRSR polling
...
#
    ...
    my @plist; # Array of sites with associated dir for processing only. No poll
	...
	if (/^[a-zA-Z]|[\d\d\d]/) {
	    ...
	    if ($info[1] eq "P") { # For processing only...
		my $line = sprintf "%s %s", $info[0],$dir;
		push (@plist, $line);
	    }
	    push (@tlist, $info[0]); # All the tli_types entries...
	    my $line = sprintf "%s %s", $info[0],$dir;
	    push (@alist, $line);
	}
    }
...
#		continue;
	...
	%h = (); # Destroy %h since we're done with it
	if ($flag == 1) {
...
# If @_ (passed as ARGV) is ZERO the module returns:
#   all valid TLI_TYPE and DIRECTORY as ref \@vlist
#   all unique TLIs as ref \@tli
#   all TLI_TYPE for processing only as ref \@plist
#
    if (scalar@_ < 1) {return (\@vlist,\@tli,\@plist);}
...
sub usage2 () {
    ...
    if ($cnt == 1) {
	$p1 = "TLI is";
	$p2 = "Three Letter Indentifier";
	$p3 = "station was";
    } else {
	$p1 = "TLIs are";
	$p2 = "Three Letter Identifiers";
	$p3 = "stations were";
    }
