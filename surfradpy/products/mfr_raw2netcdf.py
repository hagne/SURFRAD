...
class MfrsrRawToNetcdf(prowo.WorkplannerDaily):
    ...
    def process_row(self, row = None, iloc = None, loc = None,
                    id_mismatch_error = True, save = True):
        ...
        #######
        ds_rawlist = []
        for fp in row.p2f_in:
            if self.verbose:
                print(f'open file: {fp}')
            try:
        ...
        logger_ids = [ds.logger_id for ds in ds_rawlist]
        ...
        dsout = xr.concat(ds_rawlist, 'datetime')
        ...
        dep_df = dep_df[dep_df.Date_start <= start ] # remove deployments after the file date
        assert(dep_df.shape[0]>0), f'No deployment found for instrument {db_meta.Instrument} before {start}.'
        deployment = dep_df.iloc[-1]     #last_install_before_file_date

        self.tp_deployment = deployment
        assert(deployment.Date_stop == 'present' or pd.to_datetime(deployment.Date_stop) >= start), f'Instrument {db_meta.Instrument} was not deployed at {start}. Deployment ended at {deployment.Date_stop}.'

        ## Double check that the deployment site matches the expected site

        assert(deployment.Location.lower() == self.site.lower()), f'Instrument {db_meta.Instrument} was not deployed at site {self.site} at {start}, but at {deployment.Location}.'
        ...
        attrs['parent_files'] = ', '.join([d.attrs['path2file'] for d in ds_rawlist])
        attrs['day_complete'] = row.day_complete.__str__()
        dsout.attrs = attrs
